      ...
      <div className="mx-auto max-w-3xl">
        ...
        <form action={createOpportunityFromForm} className="mt-6 grid gap-5 rounded-2xl border border-slate-200/60 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="grid gap-5 sm:grid-cols-3">
            ...
            <label className="grid gap-2 text-xs font-bold uppercase tracking-wider text-slate-500">Cierre<input name="closesAt" type="date" min={new Date(Date.now() + 86_400_000).toISOString().slice(0, 10)} className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-normal normal-case tracking-normal text-slate-800 outline-none transition-all focus:border-sky-400 focus:ring-2 focus:ring-sky-100" /></label>
          </div>
