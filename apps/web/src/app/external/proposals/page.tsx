...
const STATUS_LABEL: Record<string, string> = { pending: "Pendiente", accepted: "Aceptada", rejected: "Rechazada", withdrawn: "Retirada" };

export default async function ExternalProposalsPage({ searchParams }: { searchParams?: { error?: string; updated?: string } }) {
  ...
  const { data, error } = await supabase
    ...
    .select("id, cover_letter, proposed_amount, status, created_at, applicant_id, applicant:profiles!opportunity_proposals_applicant_id_fkey(name, avatar, specialty), opportunities!inner(id, title, publisher_id)")
  ...
  return (
    <PageLayout><section className="min-h-full bg-cl-surface px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        {(searchParams?.error || error) && <p role="alert" className="mt-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{searchParams?.error ?? "No se pudieron cargar las propuestas."}</p>}
        <div className="mt-6 grid gap-5">
          {(data ?? []).map((proposal: any) => (
            <article key={proposal.id} className="rounded-2xl border border-slate-200/60 bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-bold text-slate-900">{proposal.opportunities?.title ?? "Encargo"}</h2><p className="mt-1 text-sm font-semibold text-slate-700">{proposal.applicant?.name ?? "Postulante"}{proposal.applicant?.specialty ? ` · ${proposal.applicant.specialty}` : ""}</p></div><span className="rounded-full bg-sky-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-sky-700">{STATUS_LABEL[proposal.status] ?? proposal.status}</span></div>
