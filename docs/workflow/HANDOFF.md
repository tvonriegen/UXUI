...
- **Base:** `main` / `origin/main` en `be3ed9e`. `main` sigue siendo ancestro de `HEAD`, por lo que la promoción a `main` puede ser un **fast-forward directo** (`git merge --ff-only`), sin merge commit.
- **Trabajo local sin commitear (10 archivos de app — dashboard tolerante / logout — + `package.json`):**
  - `apps/web/src/lib/auth-context.tsx` — logout tolerante (no re-lanza si `signOut` falla) y guardas de sesión (`sessionVersion` / `activeSessionId` refs + dedupe de carga) para que una respuesta de perfil obsoleta no restaure al usuario anterior tras logout o cambio de cuenta.
  - `apps/web/src/lib/role-context.tsx` — lecturas/escrituras de notificaciones tolerantes a errores (fallan cerrado a vacío, no rompen el shell).
  - `apps/web/src/components/dashboard/DashboardEstudiante.tsx` — el perfil es la única dependencia obligatoria; feed/badges/gamificación opcionales fallan cerrado a vacío sin bloquear el dashboard.
  - `apps/web/src/components/dashboard/TrustTriangleInsights.tsx` — manejo de errores en las consultas agregadas de confianza/reputación.
  - `apps/web/src/components/gamification/DailyQuestsCard.tsx` — RPC opcional; el error se degrada a "no disponible temporalmente" sin error global.
  - `apps/web/src/components/radar/TechRadarCard.tsx` — tablas/RPCs de radar ausentes toleradas en staging parcial; toast informativo en refresh.
  - `apps/web/src/app/api/streak/touch/route.ts` — 503 genérico sin exponer detalles del servicio/credenciales.
  - `apps/web/src/components/layout/TopNavBar.tsx`, `apps/web/src/components/settings/SettingsPage.tsx`, `apps/web/src/components/ui/ConfigModal.tsx` — logout en `try/catch` + `router.replace("/login")` aunque `signOut` falle; `ConfigModal` incorpora el router.
  - `package.json` — añade el script `verify:storage-migration`.
- **Nuevos sin trackear:**
  - `supabase/migrations/20260813000001_reconcile_storage_buckets.sql` — migración Storage forward-only e idempotente (metadatos de buckets `avatars`, `banners`, `post-media` + policies RLS de `storage.objects`; nunca borra ni modifica objetos).
  - `scripts/verify-storage-migration.mjs` — verificación estructural de la migración (buckets, policies owner, invariantes no destructivos).
  - `docs/technical/STORAGE_DRIFT_REPAIR.md` — documenta la reparación del drift.
- **Storage / staging:** la reparación de Storage fue **aplicada previamente al proyecto TalentHub Staging** (fuera del flujo de migraciones de este repo, según `STORAGE_DRIFT_REPAIR.md`); la migración `20260813000001` versiona esa reparación para que el contrato Git sea repetible. Según ese mismo documento, la migración **no debe aplicarse a un proyecto remoto desde este worktree** como parte de la verificación local; se aplica por el flujo de migración normal cuando el operador de staging lo decida. El estado remoto de staging/producción **no fue re-verificado** en esta sesión.
- **Concern Storage independiente:** tiene gate propio y no abre Gate D de Core. Se conserva `public=true` para descargas por URL conocida; la validación remota continúa pendiente.
- **Plan explícito de promoción (pactado, NO ejecutado en esta sesión):**
  1. Commits atómicos (un concern por commit, según `docs/git/COMMIT_CONVENTION.md`): (a) Storage — migración + `verify:storage-migration` + `package.json` + `STORAGE_DRIFT_REPAIR.md`; (b) `fix:` — los 10 archivos de dashboard tolerante / logout; (c) `docs:` — esta actualización de continuidad.
  2. Verificación local sobre los commits: `lint`, `typecheck`, `test`, `build`, `test:release`, `verify:release`, `git diff --check`.
  3. **Promoción directa fast-forward a `main`** (`git checkout main && git merge --ff-only stabilization/release-readiness`) y push — sólo con autorización explícita; nunca automático.
  4. **Limpieza posterior:** borrar la rama ya fusionada (local y remota) y actualizar `STATUS.md` / `NEXT_ACTIONS.md` / `KNOWN_ISSUES.md` al estado post-promoción.
- **No verificado en esta sesión:** comportamiento en runtime de los cambios locales, CI Node 22, estado remoto de Supabase (producción y staging), y la aplicación de `20260813000001`. Ninguna de esas afirmaciones se hace aquí.
...
- 2026-08-13 — `docs`: added the "Current State (2026-08-13)" block at the top of this file reflecting the actual repository state: branch `stabilization/release-readiness` at `ef3b428` (already pushed, in sync with `origin`, 0 ahead / 0 behind), 10 commits over `main`, fast-forward promotion possible; uncommitted local work = 10 dashboard-tolerante/logout app files plus `package.json` (new `verify:storage-migration` script); untracked = storage migration `20260813000001_reconcile_storage_buckets.sql`, `scripts/verify-storage-migration.mjs` and `docs/technical/STORAGE_DRIFT_REPAIR.md`; the storage repair was previously applied to the TalentHub Staging project and the migration versions it for a repeatable Git contract; explicit plan of atomic commits, direct fast-forward promotion to `main` and post-promotion cleanup. Only git read commands and `git diff --check` were run in this session; no lint/typecheck/test/build/`verify:*`/SQL/migration was executed. The 2026-08-05 header and the rest of the document are preserved as historical traceability.
