...
| PR 1 | fix: privacy contact routing (minor students via school) | `fix/privacy-contact-routing` | `caro-maturana` (post-PR 0 fast-forward) | **Implemented locally / validated / security approved / not committed** | **Passed** (`npm run verify:is-minor` ✓ 7/7, `npm run typecheck` ✓, `npm run lint` ✓, `npm run build` ✓ no dummy env) | not assigned | Route company contact with minor students through the school-mediated path. See `DECISION_LOG.md` ADR-002 and resolved `OPEN_QUESTIONS.md` Q12–Q14. Security review verdict after B1 / M1 fixes: APROBAR, no BLOCKER / HIGH. Runtime Supabase migration smoke test is a recommended follow-up, not a blocker. |
